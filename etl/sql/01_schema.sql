...
-- Recreate this composite index so existing databases converge on the batch_id-first order.
DROP INDEX IF EXISTS audit.idx_etl_run_log_status_batch_id;
CREATE INDEX IF NOT EXISTS idx_etl_run_log_status_batch_id ON audit.etl_run_log(batch_id, status);
