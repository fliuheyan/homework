...
Dashboard 应结合告警规则，避免只“看板可见”而无法及时响应。以下阈值仅作为起始示例，实际项目中应根据 **ETL 调度频率、历史基线和业务 SLA** 做参数化配置。一个实用的起点是：**时延类告警阈值 = 2 × 调度周期**，质量类和成功率类阈值则基于历史表现进行校准。

- 超过 `2 × scheduling_interval` 仍无成功 load（例如每 15 分钟调度一次时，可先设为 30 分钟）
- Backlog Size 超过阈值
- Job Success Rate 低于 95% （建议按日或滚动 24 小时窗口统计，示例阈值应结合业务容忍度调整）
- 数据异常率高于 3% （建议按单批次或日维度统计，可参考 `etl/data_check.py` 的历史异常分布，用 p95 异常率加安全边际作为初始阈值）
